...
// Helper function to get shape by ID across all categories
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const getShapesByType = (type: any) => {
  return getAllShapes().filter(shape => shape.type === type);
};

// Helper function to get AWS shapes by category
export const getAWSShapesByCategory = (category: string) => {
  const awsCategory = shapeCategories.aws;
  return awsCategory.shapes.filter(shape => shape.category === category);
};